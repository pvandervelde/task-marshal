...
**Rationale:** Correctness and testability outweigh the minor inefficiency of fetching all sources. Priority ordering across sources requires seeing all candidates anyway to guarantee the globally best task is returned. A further correctness argument: fetching all sources upfront means `list` and `next` observe the same candidate set — what `list` shows as "highest priority" matches what `next` returns, a consistency guarantee that lazy/streaming selection would break.
